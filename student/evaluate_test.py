    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(prompts), batch_size), desc="Batches"):
            ...
            for output in outputs:
                ...
                pred_text = pred_text[len(batch_prompts[0]):].strip() \
                    if len(batch_prompts) > 0 else pred_text.strip()
                predictions.append(pred_text)
...
def print_results(results):
    ...
    print(f"Partial Accuracy: {results['partial_accuracy']:.4f} ({results['partial_accuracy']*100:.2f}%)")
    print("="*60 + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_data_path", type=str, required=True, help="Path to test data JSONL file")
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size for generation")
    parser.add_argument("--max_tokens", type=int, default=512, help="Max tokens to generate")
    parser.add_argument("--use_vllm", action="store_true", default=True, help="Use vLLM for fast generation")
